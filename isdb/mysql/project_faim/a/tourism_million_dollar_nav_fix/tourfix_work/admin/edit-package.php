...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrf_validate($_POST['csrf_token'] ?? null)) {
        flash_set('danger', 'Security token expired.');
    } else {
        $pkg = $packages[$idx];
        $pkg['title'] = trim((string)($_POST['title'] ?? $pkg['title']));
        $pkg['country'] = trim((string)($_POST['country'] ?? $pkg['country']));
        $pkg['price'] = trim((string)($_POST['price'] ?? $pkg['price']));
        $pkg['rating'] = trim((string)($_POST['rating'] ?? $pkg['rating']));
        $pkg['days'] = trim((string)($_POST['days'] ?? $pkg['days']));
        $pkg['description'] = trim((string)($_POST['description'] ?? $pkg['description']));
        $pkg['category'] = trim((string)($_POST['category'] ?? $pkg['category']));
        $pkg['details'] = array_values(array_filter(array_map('trim', preg_split('/\R/', trim((string)($_POST['details'] ?? implode("\n", $pkg['details'] ?? [])))))));
        $image = trim((string)($_POST['image'] ?? $pkg['image']));
...
<section class="arc-section mt-0">
  <div class="container">
    <div class="form-shell p-4 p-lg-5 reveal">
      ...
      <h1 class="section-title mb-3 text-animate">Edit tour package</h1>
